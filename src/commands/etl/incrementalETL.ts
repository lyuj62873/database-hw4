    ...
    try {
        await SQLiteTarget.manager.transaction(async manager => {
            ...
            if (dimCategories.length) {
                ...
                await syncRepo.upsert({ table_name: "dim_category", last_sync: new Date().toISOString() }, ["table_name"]);
            }
            ...
            if (dimCustomers.length) {
                ...
                await syncRepo.upsert({ table_name: "dim_customer", last_sync: new Date().toISOString() }, ["table_name"]);
            }
            ...
            if (bridgeFA.length) {
                ...
                await syncRepo.upsert({ table_name: "bridge_film_actor", last_sync: new Date().toISOString() }, ["table_name"]);
            }
            ...
            if (bridgeFC.length) {
                ...
                await syncRepo.upsert({ table_name: "bridge_film_category", last_sync: new Date().toISOString() }, ["table_name"]);
            }
            ...
            if (factsPayment.length) {
                ...
                await syncRepo.upsert({ table_name: "fact_payment", last_sync: new Date().toISOString() }, ["table_name"]);
            }
        });
